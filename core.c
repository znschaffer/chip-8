...
inline bool *get_screen(struct Core *state) { return state->screen; }
...
static inline void execute_opcode(struct Core *state, uint16_t op) {
  ...
  case 0x00:
    ...
    break;
  case 0x01: {
    uint16_t addr = (second_digit << 8) | (third_digit << 4) | fourth_digit;
    state->pc = addr;
    if (DEBUG) {
    ...
    for (int y_line = 0; y_line < fourth_digit; y_line++) {
      ...
      for (int x_line = 0; x_line < 8; x_line++) {
        if ((pixels & (0b10000000 >> x_line)) != 0) {
          uint16_t x = (x_coordinate + x_line) % SCREEN_WIDTH;
          uint16_t y = (y_coordinate + y_line) % SCREEN_HEIGHT;
